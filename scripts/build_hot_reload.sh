...
# Build the game.
odin build . -use-separate-modules -extra-linker-flags:"$EXTRA_LINKER_FLAGS" -show-timings -define:RAYLIB_SHARED=true -collection:third_party=third_party -build-mode:dll -out:build/game_tmp$DLL_EXT -debug $VET
...
if ! pgrep game.bin > /dev/null; then
    odin build main_hot_reload -use-separate-modules -out:build/game.bin $VET -debug -collection:third_party=third_party
fi
